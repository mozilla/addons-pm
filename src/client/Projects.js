...
class Projects extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({projects: this.buildMetaData(projectData), team: teamData });
  }
  ...
  parseProjectMeta(HTML) {
    ...
    const engineers = doc.evaluate("//details//dl/dt[contains(., 'Engineering')]/following-sibling::dd[1]", doc, null, 2, null).stringValue.replace(/ ?@/g, '').split(',');
    const goalType = doc.evaluate("//details//dl/dt[contains(., 'Goal Type')]/following-sibling::dd[1]", doc, null, 2, null).stringValue.toLowerCase();
    const size = doc.evaluate("//details//dl/dt[contains(., 'Size')]/following-sibling::dd[1]", doc, null, 2, null).stringValue;
  ...
  buildMetaData(projectData) {
    ...
    const augmentedProjects = newProjectData.data.organization.projects.nodes.map((project, idx) => {
      ...
      const todoCount = (todoColumn && parseInt(todoColumn.node.cards.totalCount, 10)) || 0;
      const inProgressCount = (inProgressColumn && parseInt(inProgressColumn.node.cards.totalCount, 10)) || 0;
      const doneCount = (doneColumn && parseInt(doneColumn.node.cards.totalCount, 10)) || 0;
      ...
      project.meta = { ...meta, todoCount, inProgressCount, doneCount, donePerc, inProgressPerc };
      return project;
    });
  ...
  render() {
    ...
    if (projectsData.data) {
      ...
      projects = projectsData.data.organization.projects.nodes.map((project, idx) => {
        ...
        if (teamData.data) {
          for (const engineer of project.meta.engineers) {
            const foundMember = teamData.data.organization.team.members.nodes.find((item) => {
        ...
        if (engineer && project.meta.engineers && !project.meta.engineers.includes(engineer)) {
          return null;
        }

        const projectNode = (
          <div className="card-wrapper" key={idx}>
            { !projectType && currentProjectType === null ? <h2 className="project-type">Primary</h2> : null }
            { !projectType && currentProjectType === 'primary' && project.meta.goalType === 'secondary' ? <h2 className="project-type">Secondary</h2> : null }
            <Card bg={ project.meta.goalType === "primary" ? "muted" : "light" }>
              ...
              <ProgressBar>
                <ProgressBar variant="success" now={project.meta.donePerc} key={1} />
                <ProgressBar variant="warning" now={project.meta.inProgressPerc} key={2} />
              </ProgressBar>
              <Card.Body dangerouslySetInnerHTML={{__html: sanitize(project.bodyHTML)}} />
              <Card.Footer bg="light">
                <span className="updated float-left">
                  <Octicon icon={Clock} /> Updated <TimeAgo date={project.updatedAt} />
                </span>
                <Button href={project.url} size="sm" className="float-right" variant="outline-primary" target="_blank" rel="noreferrer noopener">View Project</Button>
              </Card.Footer>
            </Card>
            <br />
          </div>
    ...
    return (
      <div>
        ...
        <Navbar variant="muted" bg="light" className="shadow-sm d-flex justify-content-between" sticky="top">
          <BreadcrumbNav />
          <Nav variant="pills">
            <Nav.Item>
            ...
            <Nav.Item>
              <LinkContainer to={`/${year}/${quarter}/`} exact>
                <Nav.Link active={`/${year}/${quarter}/` === this.props.location.pathname}>All Goals</Nav.Link>
              </LinkContainer>
            </Nav.Item>
          </Nav>
        </Navbar>
        <Container fluid>
          <Row>
            ...
            <br />
            <h1>Projects for {quarter} {year} { projectType ? `(${projectType})` : engineer ? `(${engineer})` : "(All)" }</h1>
            <br />
              { (projectsData.data === null) ? <p>Loading...</p> : null }
            { projects && projects.length ? <CardDeck>{projects}</CardDeck> : (projects && projects.length === 0 ? <p>There are no Projects available for this quarter yet</p> : null) }
            </Container>
          </Row>
        </Container>
      </div>
